...
module OpenAi
  ...
  class AssistantToolsFileSearchFileSearch
    ...
    attr_accessor :max_num_results

    attr_accessor :ranking_options
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenAi::AssistantToolsFileSearchFileSearch` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenAi::AssistantToolsFileSearchFileSearch`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # @param [Object] max_num_results Value to be assigned
    def max_num_results=(max_num_results)
      if max_num_results.nil?
        fail ArgumentError, 'max_num_results cannot be nil'
      end

      if max_num_results > 50
        fail ArgumentError, 'invalid value for "max_num_results", must be smaller than or equal to 50.'
      end

      if max_num_results < 1
        fail ArgumentError, 'invalid value for "max_num_results", must be greater than or equal to 1.'
      end
