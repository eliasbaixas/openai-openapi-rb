  ...
  class CreateVectorStoreRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenAi::CreateVectorStoreRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenAi::CreateVectorStoreRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def file_ids=(file_ids)
      ...
      if file_ids.length > 500
        fail ArgumentError, 'invalid value for "file_ids", number of items must be less than or equal to 500.'
      end
    ...
    def hash
      [file_ids, name, expires_after, chunking_strategy, metadata].hash
    end
