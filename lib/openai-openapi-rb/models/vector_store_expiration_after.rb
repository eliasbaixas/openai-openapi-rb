  ...
  class VectorStoreExpirationAfter
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenAi::VectorStoreExpirationAfter` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenAi::VectorStoreExpirationAfter`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def anchor=(anchor)
      validator = EnumAttributeValidator.new('String', ["last_active_at"])
    ...
    def days=(days)
      ...
      if days > 365
        fail ArgumentError, 'invalid value for "days", must be smaller than or equal to 365.'
      end

      if days < 1
        fail ArgumentError, 'invalid value for "days", must be greater than or equal to 1.'
      end
